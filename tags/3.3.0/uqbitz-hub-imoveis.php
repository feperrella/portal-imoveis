...
function uqbhi_register_post_type_and_taxonomies() {
    ...
    /* ── CPT: imovel ── */
    register_post_type( 'uqbhi_imovel', array(
        'labels' => array(
            'name'                  => 'Imóveis',
    ...
    register_taxonomy( 'uqbhi_tipo', array( 'uqbhi_imovel' ), array(
        'labels' => array(
            'name'              => 'Tipos',
            'singular_name'     => 'Tipo',
            'menu_name'         => 'Tipos',
    ...
    register_taxonomy( 'uqbhi_finalidade', array( 'uqbhi_imovel' ), array(
        'labels' => array(
            'name'              => 'Finalidades',
    ...
    register_taxonomy( 'uqbhi_cidadebairro', array( 'uqbhi_imovel' ), array(
        'labels' => array(
...
function uqbhi_render_imovel( $post ) {
    ...
    // CEP
    $cep = uqbhi_extract_cep( $location ?: '' );

    // Montar XML do imóvel
    $x  = '    <Imovel>' . "\n";
    $x .= '      <codigoAnuncio>' . uqbhi_cdata( $id ) . '</codigoAnuncio>' . "\n";
    $x .= '      <codigoReferencia>' . uqbhi_cdata( $referencia ?: $id ) . '</codigoReferencia>' . "\n";
    ...
    if ( $email_contato )    $x .= '        <emailContato>' . uqbhi_cdata( $email_contato ) . '</emailContato>' . "\n";
    if ( $nome_contato )     $x .= '        <nomeContato>' . uqbhi_cdata( $nome_contato ) . '</nomeContato>' . "\n";
    if ( $telefone_contato ) $x .= '        <telefoneContato>' . uqbhi_cdata( $telefone_contato ) . '</telefoneContato>' . "\n";
    $x .= '      </publicador>' . "\n";
    ...
    // Características
    $x .= '      <caracteristicas>' . "\n";
    if ( $rooms )    $x .= '        <caracteristica><id>' . uqbhi_cdata('CFT2') . '</id><nome>' . uqbhi_cdata('PRINCIPALES|QUARTO') . '</nome><valor>' . uqbhi_cdata( intval($rooms) ) . '</valor></caracteristica>' . "\n";
    if ( $suits )    $x .= '        <caracteristica><id>' . uqbhi_cdata('CFT4') . '</id><nome>' . uqbhi_cdata('PRINCIPALES|SUITE') . '</nome><valor>' . uqbhi_cdata( intval($suits) ) . '</valor></caracteristica>' . "\n";
    if ( $bathroom ) $x .= '        <caracteristica><id>' . uqbhi_cdata('CFT3') . '</id><nome>' . uqbhi_cdata('PRINCIPALES|BANHEIRO') . '</nome><valor>' . uqbhi_cdata( intval($bathroom) ) . '</valor></caracteristica>' . "\n";
    if ( $metreage ) {
    ...
    if ( $parking )  $x .= '        <caracteristica><id>' . uqbhi_cdata('CFT7') . '</id><nome>' . uqbhi_cdata('PRINCIPALES|VAGA') . '</nome><valor>' . uqbhi_cdata( intval($parking) ) . '</valor></caracteristica>' . "\n";
    if ( $iptu )     $x .= '        <caracteristica><id>' . uqbhi_cdata('CFT400') . '</id><nome>' . uqbhi_cdata('PRINCIPALES|IPTU') . '</nome><valor>' . uqbhi_cdata( intval($iptu) ) . '</valor></caracteristica>' . "\n";
    if ( $condominium ) $x .= '        <caracteristica><id>' . uqbhi_cdata('CFT6') . '</id><nome>' . uqbhi_cdata('PRINCIPALES|CONDOMINIO') . '</nome><valor>' . uqbhi_cdata( intval($condominium) ) . '</valor></caracteristica>' . "\n";
    if ( $idade )    $x .= '        <caracteristica><id>' . uqbhi_cdata('CFT5') . '</id><nome>' . uqbhi_cdata('PRINCIPALES|IDADE') . '</nome><valor>' . uqbhi_cdata( intval($idade) ) . '</valor></caracteristica>' . "\n";
...
function uqbhi_admin_cep_script( $hook ) {
    global $post_type;
    if ( $post_type !== 'uqbhi_imovel' ) return;
...
function uqbhi_auto_assign_location_terms( $post_id ) {
    if ( get_post_type( $post_id ) !== 'uqbhi_imovel' ) return;
